...
class Agent:
    ...
    def Agent(self, numStates, numActions, alpha, gamma, epsilon, id):
        ...
        self.maxQ = 0
        self.stateVector = 0
        self.thresholds = self.initialiseThresholds(3, 0, 0, -20000)
        self.action_holder = []
        self.action_ = []
        self.dReward = 0
        self.P1M_array_D = []
    ...
    def selectTLOAction(self, hour, state, agent, objective):
        ...
        while action < self.numActions:
            ...
            if agent.getAgentID() == 2:
                if testAction - previousPowerOutput <= self.U2[12] and previousPowerOutput - testAction <= self.U2[13]:
                        valueQVector = self.VectorQTable[state][int(action)][objective_]
                        #print(valueQ)
                        if valueQVector == 0:
                            self.action_holder.append(valueQVector)
                            self.action_.append(action)
                            self.actionVector.append([0, 0])
                        else:
                            valueQ = valueQVector[vectorObjective]
                            #print(valueQ)
                            self.action_holder.append(valueQ)
                            self.actionVector.append(valueQVector)
                            self.action_.append(action)
