...
class Tree:
    ...
    def step(self, cumulative_reward):
        ...
        while self.num_expansions < 6:

            node = self.root 
            #print(self.root.done, file = self.file)
            #print("Step state", node.state, file = self.file)   
    ...
    def simulate(self, node):
        ...
        if node.done == True:
            ...
            cumulative_reward += node.reward + node.parent.reward
            #probability = node.getProbability()
            #print("Done Reward 1", node.state, file = self.file)
            #print("Node Reward 1", node.reward, file = self.file)
            #print("Done Reward", reward, file = self.file)
            #print("Node Reward", node.reward, file = self.file)
            #next_state, reward, done, __ = self.env.step(node.parent.state, node.action)
            #cumulative_reward += reward# + node.parent.reward
            #print("Done Reward", reward, file = self.file)
            #print("Node Reward", node.reward, file = self.file)
            probability = node.getProbability() * node.parent.getProbability()
            ...
            while _done_ == False:
                ...
                if self.numRewards == 3:
                    ...
                    #print("State", state, file = self.file)
                    #print("Action", action, file = self.file)
                    next_state, reward, done, __ = self.env.step(state, action)

                    prob = _node_.probability
                    
                    for child in _node_.children:
                        if action == child.action:
                            if child.reward == reward:
                                _node_ = child
                                prob = child.probability

                #print("Simulation", file = self.file)
                #print("State", state, file = self.file)
                #print("Reward", reward, file = self.file)
                #print("Cumulative Reward Before", cumulative_reward, file = self.file)
                #print("Node Reward", _node_.reward, file = self.file)
                cumulative_reward += reward + node.reward

                #print("Cumulative Reward After", cumulative_reward, file = self.file)
    ...
    def run(self):
        ...
        node = self.root
        childrenRewards = [[] for x in range(len(node.children))]
        childrenProbabilities = [[] for x in range(len(node.children))]    
        ...
        for child in node.children:            
            ...
            childrenRewards[a].append(child.rewards)
            childrenProbabilities[a].append(child.probabilities)             
