...
class Tree:
    ...
    def __init__(self, state, args, file, weights):
        ...
        self.hour = 0
        self.pnm = []
        self.node = decisionNode("Null", 0, 0, self.args, [0,0,0], False, 0, False, self.hour, self.pnm)
        self.root = self.node
        self.CR = [0,0]
        self.health = 0
        self.state = 0
    ...
    def step(self, cumulative_reward):
        ...
        #print("Step Intro Hour", hour, file = self.file)
        #print("Root Hour", self.root.hour, file = self.file)
        ...
        def findExpansionSpot(node, cumulative_reward):  
            ...
            if node.parent == "Null":
                ...
                #print("Chance Node Children", len(node.children), file = self.file)


            if node.type == "decision" and node.done == True:
                #print("Done?",node.hour, file = self.file)
                return node

            if node.isleaf == True:                
                node, cumulative_reward = self.expand(node, cumulative_reward)
                #print("Is Leaf?",node.hour, file = self.file)
            ...
            #print("Next Node", node.hour, file = self.file)
            return findExpansionSpot(node, cumulative_reward)           

        def rollOut(node):
            self.simulate(node)

        while self.num_expansions < 44:

            node = self.root 
            #print("Find Expansion", node.hour, file = self.file)
            node = findExpansionSpot(node, cumulative_reward)
            #print("After Find Expansion", node.hour, file = self.file)
            rollOut(node)
    ...
    def scalarize_reward(self, rewards, weights):
        ...
        reward += rewards[1] * self.weights[1]
    ...
    def runSimulate(self, chanceNode, cumulative_reward):
        ...
        if self.numRewards == 3:
            state, reward, pnm, done, __ = self.env.step(chanceNode.state, 2, chanceNode.pnm, chanceNode.action, chanceNode.hour)
        else:
        ...
        if flag == 0:                      
            decisionNode = chanceNode.createChild(chanceNode, state, chanceNode.action, reward, done, reward, True, chanceNode.hour + 1, pnm)            
    ...
    def thompsonSampling(self, node, cumulative_reward):
        ...
        checkReward = []
        checkCount = []
        sample = np.random.choice(len(node.onlineRewards) + n, 11)
        ...
        for child in node.children:
            ...
            if child.timesVisited == 0:
                ...
                #minVal = randomSample
                #bestNode = child
            thompsonSamples.append(randomSample)  

        bestActions = []

        for i in range(len(thompsonSamples)):
            if thompsonSamples[i] > minVal:
                bestActions.clear()
                bestActions.append(i)
                minVal = thompsonSamples[i]
            elif minVal == thompsonSamples[i]:
                bestActions.append(i)
    ...
    def expand(self, node, cumulative_reward):
        ...
        if self.numRewards == 3:
            ...
            state, reward, pnm, done,  __ = self.env.step(node.state, self.id, node.pnm, action, node.hour)
            ...
            chance = node.createChild(node, node.state, action, [0,0,0], False, 0, False, node.hour, node.pnm)
            child = chance.createChild(chance, state, action, reward, done, 0, False, chance.hour + 1, pnm)
        else:
        ...
        #node.childrenRemaining.delete(action)  
    ...
    def updateDistributionTable(self, state, action, reward, next_state, node):
        stringReward = str(reward)
        #print("String Reward", stringReward, file = self.file)
    ...
    def simulate(self, node):
        ...
        hour = node.hour
        _pnm_ = node.pnm
        #print("Node Hour", node.hour, file = self.file)
        #print("Node Done?", node.done, file = self.file)
        ...
        if node.done == True:
            ...
            while _done_ == False:
                ...
                if self.numRewards == 3:
                    next_state, reward, pnm, done, __ = self.env.step(state, self.id, _pnm_, action, hour)
                else:
                ...
                _done_ = done
                hour += 1
                #print("Simulate Reward", reward, file = self.file)
                a += 1
    ...
    def distributionCompare(self, node, oldNode, distRewards, distProbability, cumulative_reward, startNode):
        ...
        #print(node.type, file = self.file)
        if node.type == "chance":
            xx = 0
            array = []
            ...
            #print("Old Node Dist Rewards", distRewards, file = self.file)
            #print("Old Node Dist Probabilities", distProbability, file = self.file)
            #print("Node Dstribution Table", node.distributionTable, file = self.file)
            #print("Number of Children", len(node.children), file = self.file)
            ...
            if len(node.children) > 1:               
                ...
                for child in node.children:    
                    ...
                    #print("Child Dist Rewards", child.distribution['reward'], file = self.file)
                    #print("Child Dist Probabilities", child.distribution['probability'], file = self.file)
                    #print("Child Dstribution Table", child.distributionTable, file = self.file)
                    #print("Number of Children", len(node.children), file = self.file)                
                    if child == oldNode:
                        pass                        
            ...
            #print("Check Dist Rewards", distRewards, file = self.file)
            #print("Check Dist Probabilities", distProbability, file = self.file)

            #print(holderRewards, file = self.file)
            #print(holderProbabilities, file = self.file)
    ...
    def distributionBackPropogation(self, node, cumulative_reward):
        ...
        a = 0
        n = 10
        #print("In Backprop...", file = self.file)
        probability = 1
        distProbability = [1]
        startNode  = node
        oldNode = node.parent
        ...
        while node != self.root:
            ...
            #print("Root Type",self.root.type, file = self.file)
            #print("Node Parent", node.parent, file = self.file)
            #print("Node Dist Table", node.distributionTable, file = self.file)
    ...
    def takeAction(self, _id_, action, cumulative_reward, hour):
        
        node = self.root
        #print("Node Hour:", node.hour, "Actual Hour", hour, file = self.file)

        if self.numRewards == 3:
            next_state, reward, pnm, done, __ = self.env.step(node.state, _id_, node.pnm, action, node.hour)
        else:
        ...
        if check == 0:
            chanceNode = node.createChild(node, node.state, action, [0,0,0], done, [0,0,0], True, node.hour , node.pnm)
            self.updateDistributionTable(chanceNode.state, chanceNode.action, reward, next_state, chanceNode)
        ...
        if len(chanceNode.children) > 0:
            ...
            flag = 1
            decisionNode = chanceNode.createChild(chanceNode, next_state, action, reward, done, reward, True, chanceNode.hour + 1, pnm)
        ...
        if flag == 0:     
            ...
            #_node_ = node.createChild(node, next_state, action, [0,0,0], done, [0,0,0], True)
            decisionNode = chanceNode.createChild(chanceNode, next_state, action, reward, done, reward, True, chanceNode.hour + 1, pnm)
            #node.parent.childChanceRewards[node.action].append(reward)
            #print("New Node Tyoe", newNode.type, file = self.file)
...
class chanceNode:
    ...
    def __init__(self, node, state, action, args, _type_, hour, pnm):
        self.numActions = 11
        ...
        self.onlineRewards = np.tile(np.array([0, 0, 0]),(n, 1))
        self.count = np.empty(n)
        self.count.fill(1)

        self.artificialRewards = []
        #artificialData =  np.array([artificialData])
        r1 = np.random.randint(-4.00000000e6, 0, size=(an, 1))
        #r1.fill(-4)
        r2 = np.random.randint(-8.00000000e6, 0, size=(an, 1))
        #r2.fill(0)
        r3 = np.random.randint(-10.00000000e8, 0, size=(an, 1))
        ...
        self.data = {}
        self.distribution = {}
        self.childrenRemaining = np.arange(11)
    ...
    def createChild(self, node, state, action, reward, done, health, _type_, hour, pnm):
        #node, node.state, action, node.row, node.col
       

        child = decisionNode(node, state, action, self.args, reward, done, health, _type_, hour, pnm)
...
class decisionNode:
    ...
    def __init__(self, node, state, action, args, reward, done, health, _type_, hour, pnm):
        ...
        #self.artificialRewards = np.tile(np.array(np.array([random,randint(0, 124), 0, -random.randint(0, 30)]),(an, 1))
        self.artificialRewards = np.hstack((r1,r2,r3))
        #self.artificialRewards = np.tile(np.array(np.array([10000000000000, 0, 0]),(an, 1)))
    ...
    def createChild(self, node, state, action, reward, done, health, _type_, hour, pnm):
        ...
        child = chanceNode(node, state, action, self.args, _type_, hour, pnm)
...
class Learner(object):
    ...
    def selectAction(self, method, tree, rewards, probabilities, cumulative_rewards, cumulative_probability, state):
        ...
        if method == "Bootstrap Thompson":
            ...
            means = [[] for x in range(11)]
            thompsonSamples = []
            #print("Node type...", node.type, file = self.debug_file)
            #print(len(node.children), file = self.debug_file)
            sample = np.random.choice(len(node.onlineRewards), 11)
            #print(len(node.children), file = self.debug_file)
            #print(node.isleaf, file = self.debug_file)
            #print(node.done, node.hour, file = self.debug_file)
            for child in node.children:
                ...
                thompsonSamples.append(self.scalarize_reward((randomSample) + cumulative_rewards, self.weights) )
    ...
    def run(self, param):
        ...
        #print(env_state, file = self.debug_file)
        done = False
        self._num_rewards = 3
        ...
        start = time.time()
        self.hour = 0
        print("Root ",self.tree.root, file = self.debug_file)
        while not self.done:
            ...
            #print("Action", action, "Hour", self.hour, file = self.debug_file)
            env_state, rewards, node, done, probability = self.tree.takeAction(self.id, action, cumulative_rewards, self.hour)
            print("Hour", self.hour, "Done", done, file = self.debug_file)
