...
public class Agent {
	...
	public Double U3[] = {73.0, 340.0, 1049.9977, 40.3965, 0.0280, 320.0, 0.038, 300.3910, -4.0695, 0.0509, 0.4968, 0.0202, 80.0, 80.0};
	...
	public int numPercentSteps = 101;
	
	Environment newEnvironment = new Environment();
	
	
	public Agent(double alpha, double gamma, double epsilon, int id)
	...
	public int getStateMARL(int hour, Agent agent, double power_, int action) {
		int state = 0;
		//System.out.println(demandRange);
		int deltaDemand = deltaDemands.get(hour - 1) + demandOffset; // add demand offset so state no is always >= 0
		int power = (int) (power_ - genOffsets[agent.getAgentID() - 1])*numPercentSteps / (1 * genRanges[agent.getAgentID() - 1]);
